"""
cscv_gap_itsm.py
CSCV / PBO wired to strategy_gap_itsm.run_backtest().
"""
...
from strategy_gap_itsm import run_backtest

ALLOWED_KEYS = {'itsm_bars', 'itsm_threshold', 'atr_period', 'sl_atr_mult', 'vol_lookback', 'fixed_contracts', 'norm_threshold_k'}
...
def run_cscv(M: np.ndarray, S: int = 16) -> dict:
    ...
    is_srs   = np.array(is_srs)
    oos_srs  = np.array(oos_srs)
    sel_vars = np.array(sel_vars)
    return {
